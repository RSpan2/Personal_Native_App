        ...
        <ScrollView
          ...
          contentContainerStyle={{ paddingTop: 10 }}>
            ...
            {data?.clock?.inIntermission ? (
                <Text className="text-yellow-400 text-3xl font-bold text-center">INT</Text>
            ) : (
                <>
                    <Text className="text-red-500 text-3xl font-bold text-center">
                        {data?.periodDescriptor?.periodType === 'OT'
                            ? 'OT'
                            : data?.periodDescriptor?.periodType === 'SO'
                            ? 'SO'
                            : ['1st', '2nd', '3rd'][data?.periodDescriptor?.number - 1] ?? 'LIVE'}
                    </Text>
                    <Text className="text-white text-3xl font-bold text-center">{data?.clock?.timeRemaining}</Text>
                </>
            )}
